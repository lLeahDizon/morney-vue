...
module.exports = {
  ...
  chainWebpack: config => {
    ...
    config.module
      ...
      .use('svg-sprite-loader-mod').loader('svg-sprite-loader-mod').options({extract: false}).end() // 使用 svg-sprite-loader 这个插件，extract: false 不要把它解析出文件来
      .use('svgo-loader').loader('svgo-loader') // o 是优化 optimization 的缩写
      .tap(options => ({...options, plugins: [{removeAttrs: {attrs: 'fill'}}]})).end() // 检测如果加载了 svg ，就把 fill 属性删除
